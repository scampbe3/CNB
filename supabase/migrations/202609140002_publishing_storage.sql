create function public.publish_content(payload jsonb, hash text, actor uuid default null) returns uuid
...
begin
 ...
 -- Missing records are archived, never deleted; bookmarks and RSVPs retain their IDs.
 ...
 delete from public.portal_content;
...
insert into storage.buckets(id,name,public,file_size_limit,allowed_mime_types) values
 ('member-avatars','member-avatars',false,5242880,array['image/jpeg','image/png','image/webp']),
 ('member-resources','member-resources',false,7340032,array['application/pdf'])
 on conflict(id) do nothing;
create policy avatar_read on storage.objects for select to authenticated using(bucket_id='member-avatars' and public.is_active() and exists(select 1 from public.member_profiles where avatar_path=name));
create policy avatar_insert on storage.objects for insert to authenticated with check(bucket_id='member-avatars' and public.is_active() and (storage.foldername(name))[1]=auth.uid()::text);
create policy avatar_delete on storage.objects for delete to authenticated using(bucket_id='member-avatars' and public.is_active() and (storage.foldername(name))[1]=auth.uid()::text);
create policy asset_admin on storage.objects for all to authenticated using(bucket_id='member-resources' and public.is_admin()) with check(bucket_id='member-resources' and public.is_admin());
-- Protected downloads are authorized by a server route; no public bucket or durable URL.

create index member_terms_lookup on public.member_taxonomy_terms(term_id,member_id);
create index event_invites_member on public.event_invitations(member_id,event_id);
create index event_rsvp_waitlist on public.event_rsvps(event_id,response,updated_at);
create index discussion_by_thread on public.discussion_comments(thread_id,created_at);
create index profiles_visible on public.member_profiles(directory_visible,display_name);
