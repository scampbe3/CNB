...
Supabase organization **CNB** (`vhkbdkrwdhcpykcgvgmd`) owns project **cnb-member-portal** (`mswwugsnrhvibpzmimlc`, us-east-1). All nine migrations and 43 taxonomy terms are applied. All three Storage buckets are private. Public signup and anonymous Auth are disabled; email login remains enabled. No real member accounts were created, and no invitation/reset/event emails have been sent. Temporary hosted smoke-test identities were deleted after testing.
...
The first staging administrator invitation was sent to the authorized operator inbox on September 15, 2026. The account remains `invited` until the recipient follows the one-time email link, chooses a password, accepts the staging policies, and completes onboarding. Native portal terms and privacy pages are published under policy version `staging-draft-2026-09-15`; they are conspicuously marked for Amanda's review and must be approved or replaced before actual members are invited. No client-domain DNS changes beyond the additive Resend DKIM/SPF/MX records and no billing upgrades were made. Vercel reports this team as Hobby; the operator confirmed an eligible arrangement, but the CLI does not establish a special commercial-use exception. Confirm that arrangement before client launch.
