...
Staging is online at **https://cnb-member-portal.vercel.app/login**. Vercel project `scampbe3s-projects/cnb-member-portal` is linked to the repository, with Next.js, Node 24, root `apps/member-portal`, `npm ci` and `npm run build`. It uses the Vercel production *target* for a stable staging URL; this is not the client's production member domain. Application code baseline: `5e43e36`; subsequent documentation-only revisions do not change that tested code. The public Squarespace site and live navigation have not been changed.

Code is committed and pushed on `implementation/member-portal`, not merged into `main`. The staging project's Ignored Build Step permits only `implementation/member-portal`; other Git branches are skipped. Its automatic production branch is still `main`, so portal pushes currently produce previews, not updates to the stable staging URL. Previews deliberately have no database credentials. To enable automatic staging updates, set **Project Settings > Environments > Production > Branch Tracking** to `implementation/member-portal` in this portal project only. Do not change the GitHub repository default branch. Until then, explicitly deploy that branch to this project's production target after CI passes. See [Vercel branch configuration](https://vercel.com/docs/git#production-branch).

Supabase organization **CNB** (`vhkbdkrwdhcpykcgvgmd`) owns project **cnb-member-portal** (`mswwugsnrhvibpzmimlc`, us-east-1). All ten migrations and 43 taxonomy terms are applied. All three Storage buckets are private. Public signup and anonymous Auth are disabled; email login remains enabled. The authorized staging administrator completed onboarding and is active. Temporary hosted smoke-test identities were deleted after testing.

Verified: production build, 48 local unit/database tests, 18 desktop/mobile browser checks, and [GitHub CI on the deployed revision](https://github.com/scampbe3/CNB/actions/runs/34900500561). Hosted checks passed for password login, active-session validation, opted-out directory privacy, admin isolation and suspension revocation. Health responds 200; unauthenticated cron/media requests respond 401. Full hosted acceptance is still incomplete.

The operator has authorized the first administrator inbox. Resend's sending key is stored in ignored `.env.local` and the isolated Vercel project's sensitive production environment. DNS and a delivery test confirm `cupcakesandbroccoli.com` as a working Resend sender. Portal notifications use `The Decision Room <members@cupcakesandbroccoli.com>`. Supabase Custom SMTP is enabled through Resend with a 30-message/hour Auth limit, and the branded invitation and recovery templates are applied.

The private portal workbook has been created in the operator's Google Drive. Its ID and dedicated Google service-account credentials are configured locally and in Vercel; the service account has Viewer access. An authenticated Sheets API check confirmed all seven required tabs. The downloaded JSON credential was moved from the public-site root to ignored `apps/member-portal/.local/google-service-account.json` after validation.

The first staging administrator invitation was sent to the authorized operator inbox on September 15, 2026; the recipient chose a password, accepted the staging policies, completed a profile, and is active. Native portal terms and privacy pages are published under policy version `staging-draft-2026-09-15`; they are conspicuously marked for Amanda's review and must be approved or replaced before actual members are invited. The first private-workbook snapshot published successfully as revision `a9439523-26df-4099-9b21-f657314d2bf0` with 43 taxonomy terms and 20 content sections. No client-domain DNS changes beyond the additive Resend DKIM/SPF/MX records and no billing upgrades were made. Vercel reports this team as Hobby; the operator confirmed an eligible arrangement, but the CLI does not establish a special commercial-use exception. Confirm that arrangement before client launch.

`scripts/connect-staging.mjs <project-ref> --vercel` retrieves keys without logging them and configures the linked project's production environment only. Local credentials are in ignored `.env.local`; the generated database password is in ignored `.local/supabase-db-password.txt`. Keep these private and transfer/rotate them through an approved secret manager later. Do not expose them in chat, workbooks, screenshots or Git.

Hosted Auth settings are in `supabase/staging/supabase/config.toml`. From the repository root, run `npx.cmd supabase@latest config diff --workdir supabase/staging --project-ref mswwugsnrhvibpzmimlc` before a deliberate `config push`. Do not push the localhost config to the hosted project. To rerun the temporary-identity test, use `node --env-file=.env.local scripts/hosted-smoke.mjs mswwugsnrhvibpzmimlc --apply` from the app directory; this creates and deletes synthetic users and sends no email.
...
5. Configure Auth's Site URL to the exact staging portal origin. Allow only its `/auth/confirm` redirect plus required local development URLs. Disable public signup (`auth.enable_signup=false`), anonymous login and unused providers. Keep the Email provider enabled (`auth.email.enable_signup=true`): turning that provider flag off also blocks existing members' email logins. Require confirmed email, password length 12+, and secure password changes.
