...
Remaining connections: private Google workbook/service account and approved policy URLs/version. The authorized administrator has not yet been invited. General enrollment remains closed until those policy links are configured. No client-domain DNS changes beyond the additive Resend DKIM/SPF/MX records and no billing upgrades were made. Vercel reports this team as Hobby; the operator confirmed an eligible arrangement, but the CLI does not establish a special commercial-use exception. Confirm that arrangement before client launch.
