...
Staging is online at **https://cnb-member-portal.vercel.app/login**. Vercel project `scampbe3s-projects/cnb-member-portal` is linked to the repository, with Next.js, Node 24, root `apps/member-portal`, `npm ci` and `npm run build`. It uses the Vercel production *target* for a stable staging URL; this is not the client's production member domain. Deployed application revision: `5e43e36`. The public Squarespace site and live navigation have not been changed.
...
Remaining connections: private Google workbook/service account, verified Resend sender and Supabase SMTP/templates, approved policy URLs, and an authorized first-admin inbox. Enrollment and event mail remain unconfigured, not falsely enabled with dummy settings. No client-domain DNS changes or billing upgrades were made. Vercel reports this team as Hobby; the operator confirmed an eligible arrangement, but the CLI does not establish a special commercial-use exception. Confirm that arrangement before client launch.
