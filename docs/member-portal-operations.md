...
Staging is online at **https://cnb-member-portal.vercel.app/login**. Vercel project `scampbe3s-projects/cnb-member-portal` is linked to the repository, with Next.js, Node 24, root `apps/member-portal`, `npm ci` and `npm run build`. It uses the Vercel production *target* for a stable staging URL; this is not the client's production member domain. Deployed application revision: `c2d1d5b`. The public Squarespace site and live navigation have not been changed.
...
Verified: production build, 48 local unit/database tests, 18 desktop/mobile browser checks, and GitHub CI on the initial application commit. Hosted checks passed for password login, active-session validation, opted-out directory privacy, admin isolation and suspension revocation. Health responds 200; unauthenticated cron/media requests respond 401. Full hosted acceptance is still incomplete.
