...
The operator has authorized the first administrator inbox. Resend's sending key is stored in ignored `.env.local` and the isolated Vercel project's sensitive production environment. Resend accepted one explicitly labeled setup-test email to the operator using its test sender; inbox receipt is not yet confirmed. The key cannot list domains (`restricted_api_key`), so no custom sender domain has been verified by the operator tooling. The test sender is not configured as the portal's general `MAIL_FROM`.

Remaining connections: private Google workbook/service account, verified Resend sender and Supabase SMTP/templates, and approved policy URLs. The authorized administrator has not yet been invited. Enrollment and event mail remain unconfigured, not falsely enabled with dummy settings. No client-domain DNS changes or billing upgrades were made. Vercel reports this team as Hobby; the operator confirmed an eligible arrangement, but the CLI does not establish a special commercial-use exception. Confirm that arrangement before client launch.
