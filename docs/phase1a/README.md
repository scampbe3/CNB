...
Prepared and locally tested in the isolated `implementation/cnb-phase1a-content` branch. Asset-only commit `cef8baa` is deployed. The two new Sheet tabs and Squarespace pages are prepared. The existing `Content` tab is partially applied through row 84 after inherited validation stopped the first append attempt; version 3 resumes safely from that state. Existing CSS, loader and renderer remain unchanged.
...
- `sheet-apply-packaged.gs` is the compact version 3 resume-safe updater for a **temporary standalone Apps Script project**, never a replacement for the existing C+B Tools script. It normalizes checkbox booleans, preserves checkbox types, performs bulk reads, clears inherited validation before appending CMS rows, and downloads its immutable patch from commit `57dedcf`.
